...
class Common_model extends CI_Model
{
    ...
    function get_product($category_id,$from,$to,$order_by){
        $where  = "category_id='{$category_id}'";
        if(!empty($from)){
            $where .= "AND sell_price >= $from AND sell_price <=$to ";
        }
        $orderBy  = '';
        if(!empty($order_by)){
            $orderBy = $order_by;
        }
       // echo "SELECT * FROM product WHERE $where $orderBy";die;
        $query   = $this->db->query("SELECT * FROM product WHERE $where $orderBy");
        return $query->result_array();
