...
<div class="login-box">
  ...
  <span id="error_msg" style="display: none;" class="alert alert-danger" style="font-size: 15px;"></span>
  <div class="login-box-body">
    ...
    <form action="<?php echo site_url('admin/verifylogin')?>" method="post" role="form"
enctype="multipart/form-data" onSubmit="return validate();">
      <label for="name">Email:</label>
      <div class="form-group has-feedback">
        <input type="text" id="email" name="email" autocomplete="off" class="form-control" placeholder="Enter Registered Email" required>
        <span class="glyphicon glyphicon-envelope form-control-feedback"></span>
      </div>
     
      <div class="row">
        <div class="col-xs-4">
          <input type="submit" value="Submit" name="send" class="btn btn-primary btn-flat">
        </div>
        <div class="col-xs-8">
...
<script type="text/javascript">
  function validate(){
    ...
    var email  = $('#email').val();
    $.ajax({
            url: "<?php echo site_url('admin/verifyEmail')?>",
            data: {
                email : 'email'
            },
            type: "POST",
            success:function(result){
              if(result==0){
                ...
                $('#error_msg').text('Email Not Registered');
                return false;
