...
GROUP BY DATE_FORMAT(created_at, "%m-%Y")) as sub');
    ...
    {
      $query = $this->db->query("select * from vender_permission");
	    return $query->result();
    }

    public function feedback_get()
    {
     $query = $this->db->query("SELECT users.username,feedback.* FROM users INNER JOIN feedback  ON users.id=feedback.user_id ORDER BY feedback.created_at DESC");
